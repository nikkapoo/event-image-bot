...
if (file_exists(KPH_ICAL_IMPORTER_PATH . 'vendor/autoload.php')) {
    ...
    add_action('admin_notices', function() {
        ...
        echo __('<strong>Event Flyer Generator & Importer:</strong> The required <code>ics-parser</code> library is missing. Please run <code>composer install</code> in the plugin directory.', 'kph-ical-importer');
        echo '</p></div>';
    });
    return;
}
...
function kph_ical_importer_init() {
    ...
    if (!$adapter->is_supported_calendar_active()) {
         add_action('admin_notices', function() {
            echo '<div class="notice notice-error"><p>';
            echo __('<strong>Event Flyer Generator & Importer:</strong> No supported calendar plugin (EventON, The Events Calendar, or Modern Events Calendar) is active. Please activate one to use this plugin.', 'kph-ical-importer');
            echo '</p></div>';
        });
        return;
    }
